...
export async function fetchThreads(): Promise<ConversationVO[]> {
  const json = await requestJSON<ConversationVO[]>(`${BASE}/conversations`)
  return json.data ?? []
}
...
export async function renameThread(threadId: string, title: string): Promise<ConversationVO> {
  const json = await requestJSON<ConversationVO>(`${BASE}/conversations/${threadId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title }),
  })
...
export async function archiveThread(threadId: string): Promise<ThreadOperationResult> {
  ...
  if (!json.data) throw new Error('Conversation archive did not return updated data')
  return { ok: true, unsupported: false, operation: 'archive', threadId, message: 'success' } as any
}

export async function deleteThread(threadId: string): Promise<void> {
  await requestJSON<{ conversation_id: string }>(`${BASE}/conversations/${threadId}`, {
    method: 'DELETE',
  })
}

export async function fetchThreadMessages(threadId: string): Promise<ChatMessageVO[]> {
  const json = await requestJSON<ChatMessageVO[]>(`${BASE}/conversations/${threadId}/messages`)
  return json.data ?? []
}
...
export function streamThreadRun({
  ...
  fetchEventSource(`${BASE}/conversations/${threadId}/messages`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content: query, parent_message_id: parentMessageId ?? '' }),
    signal: ctrl.signal,
    onmessage(ev) {
